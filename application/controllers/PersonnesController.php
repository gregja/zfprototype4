...
class PersonnesController extends Zend_Controller_Action {
    ...
    public $libelle_entite = "une personne" ;
    ...
    public function indexAction() {
         ...
         */
        $typ_form_rech = $registry::get('typ_form_rech_personnes');

        /*
         * définition de la session de stockage des filtres utilisateurs
         *  attention à utiliser un namespace spécifique utilisé dans ce seul contrôleur (sinon risque de conflit)
         */
        $crud_filtres = new Zend_Session_Namespace('crud_filtres_personnes');
    ...
    public function addAction() {
        ...
        $this->view->title = 'Créer une '. $this->libelle_entite;
        ...
        if ($this->_request->isPost()) {
            ...
            if ($form->isValid($fiche_data)) {
                if ($form->getValue('submit')) {
                    $id = (int) $form->getValue('id');
                    if ($id > 0) {
                        // TODO : ajouter un contrôle interdisant de suprprimer une personne liée à un dossier
                        /*
                         * suppession physique remplacée par une suppression logique
                         */
                        $entity->delete('id = '. $id); // TODO : voir si possible de remplacer par "id = ?" et un prepared statement
                        unset($entity);
                    }
                }
                $this->_redirect('/' . $this->_request->controller);
